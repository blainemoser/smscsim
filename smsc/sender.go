...
func SendLongMessageParts(msg PDUMessage, session *Session) error {
	...
	// UDHHeader[4] = uint8(rn)         // least significant byte of the reference number - implementing 8 bits rather than 16
	...
	for i := 0; i < countParts; i++ {
		...
		if i != countParts-1 {
			messageText = append(UDHHeader, text[i*int(maxCharLen):(i+1)*int(maxCharLen)]...)
		} else {
			messageText = append(UDHHeader, text[i*int(maxCharLen):]...)
